...
namespace TownOfUs.CrewmateRoles.SnitchMod
{
    ...
    public class HighlightImpostors
    {
        private static void UpdateMeeting(MeetingHud __instance)
        {
            ...
            {
                if (Utils.PlayerById(state.TargetPlayerId).Data.Role.IsImpostor) state.NameText.color = Palette.ImpostorRed;
        ...
        public static void Postfix(HudManager __instance)
        {
            ...
            {
                if (player.Data.Role.IsImpostor) player.nameText.color = Palette.ImpostorRed;
